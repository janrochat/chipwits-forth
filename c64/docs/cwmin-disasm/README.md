...
- `forth-decompiler.md`: Notes on dictionary layout and the Go decompiler helper.
...
## FORTH decompiler helper

`c64/emulator/cmd/decompile-forth` is a small Go helper that scans the CWMIN dictionary,
locates words by name, and decompiles colon definitions using the same rules as the
`<DECOM>` / `DECOMPILE` words. See `forth-decompiler.md` for usage.
