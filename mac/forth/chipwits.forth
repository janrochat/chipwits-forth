...
0 0 342 512 Gameboard.WINDOW
W.BOUNDS
2 24 thru ( Common Ed & Game )
76 83 thru  ( anima stuff)
86 95 thru ( Ibol graphics)
sys.window select.window gameboard.window ADD.WINDOW 16
0 177 thru ( Stat&name )
96 105 thru  ( debug)
150 159 thru  ( Stack,reg stuff)
106 137 thru ( Workshop)
178 179 thru ( voc.chop)
190 192 thru
25  68 thru  ( game )
ifend gameboard.window select.window
69 71 thru
138 141 thru ( bads)
72 load ( Game)
181 186 thru ( choose.chipwit)
142 149 thru ( menu)
73 75 thru ( CHIPWITS master word)
...
( CW.load CW.save  Back.wipe Local.xy)            ( 102584 dws)
: Local.xy ( screenx/screeny---locx/locy)
  XY>point Global>Local Point>xy ;
( create grid 257 w, 257 w, 257 w, 1 c, 255 c, )
: Back.wipe ( ---|background wiper)
    0 0 512 342 gray  pattern rectangle  ( back) ;
: CW.load  ( cw# ----)
  cw.file dup ?open not if open else drop then
  robot.program swap CW.file read.fixed
  cw.file close   ;
: CW.save  ( cw# ----)
  cw.file dup ?open not if open else drop then
  robot.program swap CW.file write.fixed
  cw.file close
  ;
...
Robot.program  this.chip 2* + c@ ( Fetch op.byte)
 ...
 ;
( Get.arg false.flow get.arg.asc <sw> <gw> irnd w ( 061786 dws)
...
( Game.title draw.register)                       ( 103084 dws)
: game.title ( ---|for screen)  24 63
  0 Textfont  2dup move.to ." Score"
  2dup swap 77 + swap 7+ move.to 24 textsize ." ChipWits™"
  2dup swap 70 + swap  24 + move.to 12 textsize
  ." © 1984, DISCOURSE, Inc."
  swap 246 + swap move.to ." Cycles"  ;
: Draw.reg ( t/l/thing/size0or1---)  2 2 pensize
  >R 8*  thing.s.rect( +        ( t/l/sourcerect)
  @rect 6 pick 2+ 6 pick 6+ over 16 + over 15 + plop
  2dup swap over 27 + over 37 + 39 R> * + frame rectangle ( t/l)
  1 1 pensize
  swap 17 + over 26 + over vector
  ;
...
: Game.mouser ( ---| used only for debug   control)
  ...
  then ;
: Game.events ( ---)
     do.events case mouse.down of  Game.mouser endof endcase  ;
...
( Stop.game Leave.game Start.game)                ( 061786 dws)
...
: Init.ChipWits ( ---)  back.wipe                 ( 061786 dws)
gameboard.window 2+ sys.bmap 14 cmove  ( save screen data)
gameboard.window 2+ screen.bmap 14 cmove  ." screens saved"
...
: Series ( ---) menus.off start.game 0 last.mission ! GAME
 ...
 ( ******** ChipWits Master Program**** )         ( 040685 dws)
: ChipWits ( ACT.FL--) IF gameboard.window window Init.ChipWits
 BEGIN random drop DO.EVENTS DROP Prog.status @
...
512 w,
create back.buffer back.len@ allot
  ( scr 179 puts it in block buffer)
...
( Anim.buffer TTl.buffer and rects)               ( 061786 dws)
back.wide@ 8/ 1+ back.high@ * is anim.len@
create anim.buffer anim.len@ allot ( sc 179)
...
create ttl.buffer ttl.len@ allot ( scr 179 puts it in heap)
...
;
( Save new clipboard to "IBOL")                   ( 010104 dws)
...
5 remove " Copy of Clipboard File" 5 assign
?file.error 5 open
...
forget Pic.buffer 5 remove " IBOL" 5 assign
?file.error ( 5 create.file ( new)  5 open  IBOL.buffer@
IBOL.len 0 5 write.virtual ?file.error  5 close
( Save new clipboard to "Source Graphics")        ( 010104 dws)
create Source.buffer source.len allot ( Source graphics)
create Source.bmap source.buffer , source.width@ 8/ w,
0 w, 0 w, source.height@ w, source.width@ w,
sys.window 2+ @ constant Pic.buffer   ( store clip in screen)
7 remove " Clipboard File" 7 assign ?file.error 7 open
...
sys.bmap SetPortBits ." back"
source.bmap sys.window 2+ src.rect src.rect srccopy 0 copybits
forget Pic.buffer 8 remove " Source Graphics" 8 assign
...
( Wire.it Brancher? Draw.wire)                    ( 093084 dws)
: wire.it ( rect --- rect)
   Ibol.bmap screen.bmap 3 pick draw.rect srccopy 0 copybits ;
: Brancher? ( opcode---flag|Does it branch?)
    dup Sing@ > swap on.number@ < and ;
: Draw.wire ( action token/topleft----)
over dup Goto.go@ = swap Boomerang@ = or not
if   ( Draw some wire)
  dup Wire.rect           ( token/tøplft/rect)
   rot Brancher? ( All branchers)
   if dup 10 0 Offsetrect  ( Point to True wires in IBOL.bmap)
      wire.it -10 0 offsetrect  ( put back toys)
      False.flow wire.rect dup 20  0 offsetrect
      wire.it -20 0 offsetrect    ( False wire)
   else  ( no branch)   wire.it  drop drop  then
 else drop drop then ;
...
( Workshop.window )                               ( 071786 dws)
: Workshop.window 27 332 334 507  Window2.rect !rect ( Chipkit)
  Window2.rect @rect white pseudo.window  12 textsize
 srccopy textmode   0 textfont srcor textmode ibol.labels
 action.label.rect @ point>xy 12 + swap 40 + swap move.to
  ." OPERATORS"  thing.label.rect @  point>xy
 12 + swap 36 + swap move.to     ." ARGUMENTS"
  Title.rect frame.lab 36 textsize
 title.rect @ point>xy 34 + swap 6+ swap move.to ." IBOL"
12 textsize 85 -17 rmove ." ™"
 thing.rect shaded.lozenge  -@ ws.thing.icons  ( grayed out)
 action.rect shaded.lozenge WS.action.icons
 Garbage.rect shaded.lozenge srccopy textmode 354 40 move.to
 ."                          "   354 40 move.to
  ."  ChipWits Workshop " srcor textmode Draw.w.commands   ;
...
( ******* Workshop Master Word *********)         ( 040585 dws)
...
( Stack & reg stuff)                              ( 061786 dws)
: dam.yx 48 340 ; : fuel.yx 48 373 ; : rng.yx 87 373 ;
: key.yx 87 340 ;
: mov.yx 48 406 ; : obj.yx 48 439 ; : num.yx 48 472 ;
256 is stack.len@
create mov.st(  stack.len@ allot   ( chop.vocab assigned)
create obj.st(  stack.len@ allot
create num.st(  stack.len@ allot
...
create Stats(  game.stat.len@ allot
8 is ok.envir.len@
...
( Transient.allot Transient.return)    exit       ( 061786 dws)
...
: Small.sys.w ( ---) 0 0 1 1 sys.window w.bounds ;
configure.with small.sys.w
: startup
       (   0 0 5 10  sys.window w.bounds
          2 sys.window w.type
...
( Data in heap Snapshot prep)
Source.buffer   scr 81    10672 bytes
Ibol buffer (scr 88)       1320 bytes

stats(  ( scr 160)           80 bytes
stacks scr 150              756
 ...
 ( Robotnik loader)                               ( 071786 dws)
1 delete.menu   70800 resize.object   42890 resize.vocab
decimal
